...
if (psmethod=="gl"){
cases1=gl(cases,n,logrr,v,study,id)$cases1
n1=gl(cases,n,logrr,v,study,id)$n1
}

for (j in 1:max(id)){
if (psmethod=="h"|psmethod=="gl"){
if (study[id==j][1]=="cc"|study[id==j][1]==1){
s0<-1/cases1[id==j & v==0]+1/1/n1[id==j & v==0]
si<-s0+1/cases1[id==j & v!=0]+1/1/n1[id==j & v!=0]
}
...
rcorr<-s0/(si%*%t(si))^.5; diag(rcorr)<-1
